...
run: env-check ## Run the agent locally
	@echo "🤖 Starting meme-generator-agent..."
	@uv run python -m meme_generator_agent

.PHONY: run-dev
run-dev: ## Run the agent in development mode with auto-reload
	@echo "🤖 Starting meme-generator-agent in development mode..."
	@uv run watchfiles "python -m meme_generator_agent" meme_generator_agent/
...
docker-build: ## Build Docker image
	...
	@docker build -f Dockerfile.agent -t meme_generator_agent:latest .

.PHONY: docker-build-no-cache
docker-build-no-cache: ## Build Docker image without cache
	@echo "🐳 Building Docker image (no cache)..."
	@docker build --no-cache -f Dockerfile.agent -t meme_generator_agent:latest .

.PHONY: docker-run
docker-run: ## Run Docker container with .env file
	@echo "🐳 Running Docker container..."
	@docker run -d \
		--name meme_generator_agent \
		-p 3773:3773 \
		--env-file .env \
		--restart unless-stopped \
		meme_generator_agent:latest
...
docker-stop: ## Stop Docker container
	...
	@docker stop meme_generator_agent || true
	@echo "✅ Container stopped"

.PHONY: docker-rm
docker-rm: docker-stop ## Remove Docker container
	@echo "🗑️  Removing Docker container..."
	@docker rm meme_generator_agent || true
	@echo "✅ Container removed"

.PHONY: docker-logs
docker-logs: ## View Docker container logs
	@docker logs -f meme_generator_agent

.PHONY: docker-shell
docker-shell: ## Open shell in running Docker container
	@docker exec -it meme_generator_agent bash
...
docker-clean: docker-rm ## Clean up Docker images and containers
	...
	@docker rmi meme_generator_agent:latest || true
...
docker-push: ## Push Docker image to Docker Hub
	...
	@docker tag meme_generator_agent:latest para5/meme_generator_agent:latest
	@docker push para5/meme_generator_agent:latest
...
status: ## Show agent and Docker status
	...
	@docker ps -a | grep meme_generator_agent || echo "No containers found"
	@echo "\nDocker Images:"
	@docker images | grep meme_generator_agent || echo "No images found"
	@echo "\nAgent Health:"
